 ...
 * @param {string} name
 */
async function captureDesignScreenshot(page, subdir, name) {
  const runTs = process.env.PLAYWRIGHT_SCREENSHOT_RUN_TS
    || new Date().toISOString().slice(0, 16).replace("T", "_").replace(":", "-");
  const dir = path.join("D:\\Projects\\Sarvik\\Care-Flow\\screenshots", runTs, subdir);
