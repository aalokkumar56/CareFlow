...
Playwright functional coverage in `dotnet-frontend-test/e2e/` is generated **data-driven** from shared config in `helpers/test-matrix.js` and `helpers/routes.js`. Tests are not copy-pasted; `for` loops over role × route × filter × viewport matrices produce distinct Playwright scenarios.
